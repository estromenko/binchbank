...
var EducationWhere = struct {
	...
	ID:   whereHelperint{field: "\"educations\".\"id\""},
	Name: whereHelperstring{field: "\"educations\".\"name\""},
}
...
var (
	educationAllColumns            = []string{"id", "name"}
	educationColumnsWithoutDefault = []string{"name"}
...
func (q educationQuery) One(ctx context.Context, exec boil.ContextExecutor) (*Education, error) {
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "models: failed to execute a one query for educations")
	}
...
func (q educationQuery) Count(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "models: failed to count educations rows")
	}
...
func (q educationQuery) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	...
	err := q.Query.QueryRowContext(ctx, exec).Scan(&count)
	if err != nil {
		return false, errors.Wrap(err, "models: failed to check if educations exists")
	}
...
func (o *Education) Clients(mods ...qm.QueryMod) clientQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"clients\".\"education_id\"=?", o.ID),
	)
...
func (educationL) LoadClients(ctx context.Context, e boil.ContextExecutor, singular bool, maybeEducation interface{}, mods queries.Applicator) error {
	var slice []*Education
	var object *Education

	if singular {
		object = maybeEducation.(*Education)
	} else {
		slice = *maybeEducation.(*[]*Education)
	}
	...
	query := NewQuery(
		qm.From(`clients`),
		qm.WhereIn(`clients.education_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.EducationID {
				local.R.Clients = append(local.R.Clients, foreign)
				if foreign.R == nil {
					foreign.R = &clientR{}
				}
				foreign.R.Education = local
				break
			}
		}
	}
...
// of the education, optionally inserting them as new records.
// Appends related to o.R.Clients.
// Sets related.R.Education appropriately.
func (o *Education) AddClients(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Client) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.EducationID = o.ID
...
func Educations(mods ...qm.QueryMod) educationQuery {
	mods = append(mods, qm.From("\"educations\""))
	return educationQuery{NewQuery(mods...)}
}
...
func FindEducation(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*Education, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"educations\" where \"id\"=$1", sel,
	)
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "models: unable to select from educations")
	}
...
func (o *Education) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	if o == nil {
		return errors.New("models: no educations provided for insertion")
	}
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"educations\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"educations\" %sDEFAULT VALUES%s"
		}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to insert into educations")
	}
...
func (o *Education) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		if len(wl) == 0 {
			return 0, errors.New("models: unable to update educations, could not build whitelist")
		}

		cache.query = fmt.Sprintf("UPDATE \"educations\" SET %s WHERE %s",
			strmangle.SetParamNames("\"", "\"", 1, wl),
			strmangle.WhereClause("\"", "\"", len(wl)+1, educationPrimaryKeyColumns),
		)
	...
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to update educations row")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: failed to get rows affected by update for educations")
	}
...
func (q educationQuery) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to update all for educations")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to retrieve rows affected for educations")
	}
...
func (o EducationSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"educations\" SET %s WHERE %s",
		strmangle.SetParamNames("\"", "\"", 1, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, educationPrimaryKeyColumns, len(o)))
...
func (o *Education) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
		return errors.New("models: no educations provided for upsert")
	}
	...
	if !cached {
		...
		if updateOnConflict && len(update) == 0 {
			return errors.New("models: unable to upsert educations, could not build update column list")
		}

		conflict := conflictColumns
		if len(conflict) == 0 {
			conflict = make([]string, len(educationPrimaryKeyColumns))
			copy(conflict, educationPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"educations\"", updateOnConflict, ret, update, conflict, insert)
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert educations")
	}
...
func (o *Education) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"educations\" WHERE \"id\"=$1"
	...
	result, err := exec.ExecContext(ctx, sql, args...)
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to delete from educations")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: failed to get rows affected by delete for educations")
	}
...
func (q educationQuery) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	result, err := q.Query.ExecContext(ctx, exec)
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to delete all from educations")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: failed to get rows affected by deleteall for educations")
	}
...
func (o EducationSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"educations\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, educationPrimaryKeyColumns, len(o))
...
func (o *EducationSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"educations\".* FROM \"educations\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, educationPrimaryKeyColumns, len(*o))
...
func EducationExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"educations\" where \"id\"=$1 limit 1)"
	...
	err := row.Scan(&exists)
	if err != nil {
		return false, errors.Wrap(err, "models: unable to check if educations exists")
	}
